module Kpi
  class Entry
    ...
    def self.generated_details_data kpi, project_item, node=nil
      ...
      if node
        kpi_entries=Kpi::Entry.where(kpi_id: kpi.id, project_item_id: project_item.id, node_id: node.id)
      else
    ...
    def self.to_total_xlsx kpi_entries, kpi
      ...
      wb.add_worksheet(:name => "sheet1") do |sheet|
        ...
        if kpi
          kpi_entries.order_by(node_id: :acs).each_with_index { |kpi_entry, index|
            sheet.add_row [
                              index+1,
